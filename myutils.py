...
def getPiece(side, pos):
    if side == "w": Board = wBoard
    elif side == "b": Board = bBoard
    for i in range(len(Board)):
        if Board[i] != None and Board[i][:2] == pos:
            if i in range(8): return "pawn"
            elif i in range(8,10): return "rook"
            elif i in range(10,12): return "knight"
            elif i in range(12,14): return "bishop"
            elif i == 14: return "queen"
            elif i == 15: return "king"
            else: return Board[i][2]
...
def isOccupied(x, y, flag="w"):
    if flag == 'w':
        for i in bBoard:
            if i != None and i[:2] == [x, y]: return "b"      
        for i in wBoard:
            if i != None and i[:2] == [x, y]: return "w"
    elif flag == 'b':
        for i in wBoard:
            if i != None and i[:2] == [x, y]: return "w"
        for i in bBoard:
        ...
        if i != None:
            if a in range(8) and side == 'w':
                ...
                if i[1] == 7 and isOccupied(i[0],5) == isOccupied(i[0],6) == "empty":
                    if flag: yield [a,[i[0], 5]]
                if isOccupied(i[0], i[1]-1) == "empty":
                    if flag: yield [a,[i[0], i[1]-1]]
